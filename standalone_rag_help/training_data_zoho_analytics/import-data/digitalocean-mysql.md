Import Data from DigitalOcean MySQL
The data stored in DigitalOcean MySQL Cloud Database can be analyzed using Zoho Analytics in two ways. You can either import your data into Zoho Analytics, or connect directly with the DigitalOcean MySQL server.
- Data Import: The data in DigitalOcean MySQL will be imported and stored in Zoho Analytics. You can set up periodic schedules to fetch the latest data automatically from your DigitalOcean MySQL database. The reports loading time will load faster as the data is stored in Zoho Analytics.
- Live Connect: In this mode, data will not be fetched from DigitalOcean MySQL and stored in Zoho Analytics. Instead for the reports that you create, Zoho Analytics will generate appropriate queries that will connect the required data live from DigitalOcean MySQL to Zoho Analytics and show you the report. In this case, the loading time will directly depend on the performance of DigitalOcean MySQL.
...
Data Import
- How do I import data from the DigitalOcean MySQL database?
- How can I edit the setup?
- How long does it take for the data to be imported into Zoho Analytics?
- Can I import data from Views created in DigitalOcean MySQL (apart from Tables) into Zoho Analytics?
- Will foreign keys defined between my tables in the DigitalOcean MySQL database be linked in Zoho Analytics as well?
...
- Can I import data from DigitalOcean MySQL database into an existing table in Zoho Analytics?
- I have synced data from DigitalOcean MySQL database into a table. Can I change the data source of this table?
...
- Can I import data from my DigitalOcean MySQL database into an existing Zoho Analytics workspace?
...
Live Connect
- How do I connect live with the DigitalOcean MySQL database?
...
- Can I connect Views created in DigitalOcean MySQL (apart from Tables) to Zoho Analytics?
- Will foreign keys defined between my tables in DigitalOcean MySQL database be linked in Zoho Analytics as well?
...
- Can I connect new columns added in my DigitalOcean MySQL database to Zoho Analytics?
- Can I change the data type of the columns in Zoho Analytics?
- Can I import data from other data sources into the same workspace that I have used to connect with DigitalOcean MySQL?
- Can I create Query Tables over the DigitalOcean MySQL data?
- What happens when I delete or rename the database in DigitalOcean MySQL?
- How do I remove the Setup?
- What are the limitations of using the DigitalOcean MySQL Live Connect?
...
Data Import
1. How do I import data from the DigitalOcean MySQL database?
2. How can I edit the setup?
3. How long does it take for the data to be imported into Zoho Analytics?
After setting up, you might have to wait for sometime for the initial fetch to finish. This depends on the amount of data to be imported into Zoho Analytics and also the response time of your DigitalOcean MySQL server. You will receive an email notification once the import is complete. Please note that if you access the workspace before the initial fetch, it will not display any data.
4. Can I import data from Views created in DigitalOcean MySQL (apart from Tables) into Zoho Analytics?
Yes, you can import data from Tables as well as Views available in your DigitalOcean MySQL database into your Zoho Analytics workspace.
5. Will foreign keys defined between my tables in the DigitalOcean MySQL database be linked in Zoho Analytics as well?
When importing multiple tables, the foreign keys defined between the tables in DigitalOcean MySQL database will be linked in Zoho Analytics. The foreign keys will be created as Lookup columns in Zoho Analytics.
...
Yes, you can change the data type of the columns imported into Zoho Analytics. However, it is necessary that the data type of your column is compatible with the data type of the column in your DigitalOcean MySQL database for successful data synchronizations. It is always recommended that you change the data type in both your DigitalOcean MySQL database as well as your Zoho Analytics workspace.
...
- From your DigitalOcean MySQL workspace, navigate to Create > New Table / Import Data.
- Click the Cloud Databases option. The Connect to Cloud Database wizard will open.
- From the Connection Name drop-down menu, select the existing DigitalOcean MySQL database name.
- The rest of the steps are similar to importing data from cloud database.
8. Can I import data from DigitalOcean MySQL database into an existing table in Zoho Analytics?
...
- You can choose to import from the same table or a different table using the Select Table option, or import using the Custom Query and Stored Procedure.
- The rest of the steps are similar to importing from a single table.
9. I have synced data from DigitalOcean MySQL database into a table. Can I change the data source of this table?
Yes, you can change the data source of the [Zoho Analytics] table into which the DigitalOcean MySQL database has been synced. To do this,
- Open the workspace.
...
11. Can I import data from my DigitalOcean MySQL database into an existing Zoho Analytics workspace?
...
- Open the required workspace.
- Click the Data Sources tab from the left bar.
- All the data sources for this workspace will be listed. Click the DigitalOcean MySQL data source that you want to remove.
...
Live Connect
1. How do I connect live with the DigitalOcean MySQL database?
...
Data in DigitalOcean MySQL will be imported and stored in Zoho Analytics. | Data from DigitalOcean MySQL will be fetched live using appropriate reporting queries whenever you create or access a report in Zoho Analytics. |
Filtered data set can be imported from DigitalOcean MySQL using customized queries. | Custom Query feature is not available in DigitalOcean MySQL Live Connect. However, you can create Views in the source database and connect the same with Zoho Analytics. |
Multiple data sources (apart from DigitalOcean MySQL) can be imported into the same workspace and they can be combined for reporting & analysis purposes. | Cannot import data from any other data source in to the same workspace in which Live Connect from DigitalOcean MySQL is setup. |
...
When importing multiple tables, the foreign keys defined between the tables in the DigitalOcean MySQL database will not be linked in Zoho Analytics. However, you can manually link the tables in Zoho Analytics using the Lookup column feature. | Lookup relationship will be automatically created for tables that are linked via foreign keys in the DigitalOcean MySQL database. You can also manually link the tables in Zoho Analytics using Lookup column feature. |
Users can create query tables. | Users cannot create query tables. |
Report loading time will be fast as the data is stored in Zoho Analytics. | Report loading time directly depends on the server response time and the amount of data in DigitalOcean MySQL. |
4. How long does it take for me to visualize my data in Zoho Analytics?
As there is no data import process involved, the loading time depends upon the amount of data stored in your DigitalOcean MySQL database and also the response time of your DigitalOcean MySQL server.
...
6. Can I connect Views created in DigitalOcean MySQL (apart from Tables) to Zoho Analytics?
Yes, you can connect with both Views and Tables created in DigitalOcean MySQL to Zoho Analytics.
7. Will foreign keys defined between my tables in DigitalOcean MySQL database be linked in Zoho Analytics as well?
Yes. In case you have linked two or more tables in your DigitalOcean MySQL database using foreign keys, they will be linked automatically using a Lookup column in Zoho Analytics as well. You can also manually create a lookup relationship among the imported tables in Zoho Analytics.
...
- You will not be able to create Query Tables if you have set up the workspace using the DigitalOcean MySQL Live Connect option as this option does not fetch and store the data locally in Zoho Analytics.
...
This alert message will be displayed when Zoho Analytics is not able to access the information from DigitalOcean MySQL. This could be because the tables/columns that you are trying to access in Zoho Analytics are deleted or renamed in DigitalOcean MySQL.
...
When you have created reports in Zoho Analytics over the tables or columns which no longer have a direct mapping in DigitalOcean MySQL database, it will be listed as mismatch.
So, ensure that the tables/columns in Zoho Analytics workspace and DigitalOcean MySQL database matches. The tables/columns that do not match will be listed in the Mismatch tab of the DigitalOcean ConnectionSettings page. Refer to the next question to know more about mismatch.
...
- You will get navigated to the DigitalOcean MySQL Connection Settings page. All the tables available in the workspace will be listed.
...
13. Can I connect new columns added in my DigitalOcean MySQL database to Zoho Analytics?
Yes, you can connect the new columns that are added in your DigitalOcean MySQL database to Zoho Analytics from the DigitalOcean MySQL Connection Settings page. Refer to this presentation to know more.
...
15. Can I import data from other data sources into the same workspace that I have used to connect with DigitalOcean MySQL?
No, you cannot import data from other data sources into the same workspace that you have used to connect with DigitalOcean MySQL.
16. Can I create Query Tables over the DigitalOcean MySQL data?
No, you will not be able to create Query Tables if you have set up the workspace using the DigitalOcean MySQL Live Connect option. This is because this option does not fetch and store the data locally in Zoho Analytics. If you wish to create Query Tables, you can create the required query as a view in the data source and connect the same with Zoho Analytics.
17. What happens when I delete or rename the database in DigitalOcean MySQL?
When you delete or rename a database in DigitalOcean MySQL, Zoho Analytics loses its connectivity with DigitalOcean MySQL. After that, a warning message, as shown below, would be displayed. This error message will also be displayed if there are any connectivity issues or if your DigitalOcean MySQL credentials have expired.
...
19. What are the limitations of using the DigitalOcean MySQL Live Connect?
Given below are a few short comings that one might face while using the DigitalOcean MySQL Live Connect option.
- Data from your DigitalOcean MySQL database will NOT be locally stored in Zoho Analytics. Zoho Analytics will generate appropriate queries to fetch the required data live from DigitalOcean MySQL and show you the report. Hence, the loading time will directly depend on the performance of DigitalOcean MySQL.
...
- Users cannot combine data from other data sources into the same workspace in which you have connected the DigitalOcean MySQL database.
